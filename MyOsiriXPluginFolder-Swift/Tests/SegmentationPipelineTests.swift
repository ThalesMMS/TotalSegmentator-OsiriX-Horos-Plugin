...
final class TokenizeTests: XCTestCase {
    ...
    func test_emptyString_returnsEmptyArray() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: ""), [])
    }

    func test_singleToken_returnsOneElement() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "hello"), ["hello"])
    }

    func test_twoTokens_separatedBySpace_returnsTwoElements() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "hello world"), ["hello", "world"])
    }

    func test_multipleSpaces_betweenTokens_collapses() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "a   b   c"), ["a", "b", "c"])
    }

    func test_leadingSpaces_areIgnored() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "   token"), ["token"])
    }
    ...
    func test_quotedStringInMiddle_parsedAsOneToken() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "--arg \"my value\" --other"), ["--arg", "my value", "--other"])
    }

    func test_backslashEscape_preservesNextCharacter() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "a\\ b"), ["a b"])
    }
    ...
    func test_mixedSingleAndDoubleQuotes_insideDoubleQuotes_preservesSingleQuote() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "\"it's\""), ["it's"])
    }

    func test_mixedSingleAndDoubleQuotes_insideSingleQuotes_preservesDoubleQuote() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "'say \"hi\"'"), ["say \"hi\""])
    }

    func test_dashArguments_parsedNormally() {
        let result = TotalSegmentatorHorosPlugin.tokenize(commandLine: "--task total --device cpu")
    ...
    func test_unclosedQuote_capturesRemainingCharacters() {
        ...
        let result = TotalSegmentatorHorosPlugin.tokenize(commandLine: "a \"unclosed")
    ...
    func test_singleFlag_returnsOneToken() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.tokenize(commandLine: "--fast"), ["--fast"])
    }
}
...
final class DetectOutputTypeTests: XCTestCase {
    ...
    func test_emptyTokens_returnsDicomWithEmptyRemaining() {
        let (type, remaining) = TotalSegmentatorHorosPlugin.detectOutputType(from: [])
    ...
    func test_outputTypeFlag_withUnknownValue_producesOther() {
        ...
        let (type, _) = TotalSegmentatorHorosPlugin.detectOutputType(from: tokens)
...
final class RemoveROISubsetTokensTests: XCTestCase {
    ...
    func test_emptyTokens_returnsEmptyArray() {
        XCTAssertEqual(TotalSegmentatorHorosPlugin.removeROISubsetTokens(from: []), [])
    }

    func test_noROISubsetFlag_preservesAllTokens() {
        let tokens = ["--task", "total", "--fast"]
        XCTAssertEqual(TotalSegmentatorHorosPlugin.removeROISubsetTokens(from: tokens), tokens)
    }

    func test_roiSubsetFlag_withSingleValue_removesFlagAndValue() {
        let tokens = ["--roi_subset", "liver"]
        XCTAssertEqual(TotalSegmentatorHorosPlugin.removeROISubsetTokens(from: tokens), [])
    }
    ...
    func test_roiSubsetFlag_valuesSeparatedByNextFlag_preservesNextFlag() {
        ...
        XCTAssertEqual(TotalSegmentatorHorosPlugin.removeROISubsetTokens(from: tokens), ["--fast"])
    }
    ...
    func test_roiSubsetFlag_inMiddleOfTokens_removesOnlySubsetTokens() {
        ...
        XCTAssertEqual(TotalSegmentatorHorosPlugin.removeROISubsetTokens(from: tokens), ["--task", "total", "--device", "cpu"])
    }
    ...
    func test_roiSubsetAtEndWithNoValues_removesJustTheFlag() {
        ...
        XCTAssertEqual(TotalSegmentatorHorosPlugin.removeROISubsetTokens(from: tokens), ["--task", "total"])
    }
...
final class ResolveOutputDirectoryTests: XCTestCase {
    ...
    func test_resolveOutputDirectory_withExistingDirectory_returnsProvidedURL() throws {
        ...
        let resolved = try TotalSegmentatorHorosPlugin.resolveOutputDirectoryIfProvided(existingDir)

        XCTAssertEqual(resolved, existingDir)
    }
    ...
    func test_resolveOutputDirectory_withNonExistentDirectory_createsIt() throws {
        ...
        let resolved = try TotalSegmentatorHorosPlugin.resolveOutputDirectoryIfProvided(newDir)

        XCTAssertEqual(resolved, newDir)
    ...
    func test_resolveOutputDirectory_existingFile_notDirectory_raisesError() {
        ...
        XCTAssertThrowsError(try TotalSegmentatorHorosPlugin.resolveOutputDirectoryIfProvided(filePath)) { error in
            XCTAssertEqual((error as NSError).domain, "org.totalsegmentator.plugin")
